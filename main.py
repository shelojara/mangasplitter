...
def process_file(root: str, file: str):
    ...
    image = trim(Image.open(os.path.join(root, file)))
...
def zip_output(file: str):
    ...
    shutil.make_archive(filename + "_processed", "zip", "output")
    shutil.move(filename + "_processed.zip", filename + "_processed.cbz")
